...
text_box1 = TextBox(posbox, '<RA DEC FoV> or <messierNumber FoV>:')
...
def pos(text):
    ...
    elif len(lst) == 3 and int(lst[0]) >= 0 and int(lst[0]) < 360 and int(lst[1]) >= -90 and int(lst[1]) <= 90:
        ra = int(lst[0])
        dec = int(lst[1])
        fov = int(lst[2])
        xl = ra - fov / 2 - fov / 10
...
def onclick(event):
    ...
    print(xi, yi)
    global d
    d[ms[(ms['RAJ2000'])==l[0]]['ID (for resolver)']].append()
    if len(a) == 2:
