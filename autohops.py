...
for i in range(110):
    x = float(ms['RAJ2000'][i])
    y = float(ms['DEJ2000'][i])
    name = str(ms['ID (for resolver)'][i])
    point = np.array([x, y])
    hop_dict[name] = []
    for j in list:
        ind = tree.query_ball_point(point, j)
        if not ind == []:
            objra = mag_ty['RAJ2000'][mag_ty['V'][ind].idxmax()]
            objde = mag_ty['DEJ2000'][mag_ty['V'][ind].idxmax()]
            if not [objra, objde] in hop_dict[name]:
                hop_dict[name].append([objra, objde])
    hop_dict[name].append([x, y])
    print(hop_dict)

pickle.dump(hop_dict, open("autohop.pkl", "wb"))
